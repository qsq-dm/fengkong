...
window.ShearPhoto.MINGGE(function() {
	//██████████重要设置████████████████
	var relativeUrl = "/Public/plugins/shearphoto_common"; //你不要在后面加斜杠，系统会自动给你加上斜杠，不信看下面！   index.html的JS引用路径自己改，很简单的说
	/*
	...
	Shear.complete = function(serverdata) { //截图成功完成时，由shearphoto.php返回数据过来的成功包
			...
			//console.log(serverdata[0]['ImgName']);

		//交给php来写入数据库中
			var data = serverdata[0]['ImgName'];
			this.MyAjax.carry({
				url: 'changePhoto',
				data: 'photoName='+data,
				type: "POST",
				...
				success: function(data){
					if (data){
						var photo = opener.document.getElementById("photosrc");
						photo.src = "/Public/plugins/shearphoto_common/file/shearphoto_file/" + data.replace(/\"/g, "");
					}
				}
			});
		//交给php来写入数据库中
